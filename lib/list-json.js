...
module.exports.getList = function(options , inThis , inHexo , inForcePost) {
    ...
    if(options.PPTagFilter && options.PPTagFilter != ""){
        postsTag = options.PPTagFilter;
    }
    ...
    if( Number(options.PPMixingRate) != 0 ){
        ...
        if(postsTag != undefined && postsTag != ""){
            for( i = 0; i < _this.config.popularPosts.tmp.gaData.length; i++){
                ...
                if(_this.config.popularPosts.tmp.gaData[i].date != ""){
                    var p_tags = _this.config.popularPosts.tmp.gaData[i].tags;
                    
                    if(p_tags && p_tags.length > 0){
                        for(var pi = 0; pi < p_tags.length; pi++){
                            if(p_tags[pi] == postsTag){
        ...
        // filtering items category
            ...
            for( i = 0; i < _this.config.popularPosts.tmp.gaData.length; i++){
                if(_this.config.popularPosts.tmp.gaData[i].date != ""){
                    ...
                    var cat_spl;
                    if( _this.config.popularPosts.tmp.gaData[i].categories.match(/\s\>\s/) ){
                        cat_spl = _this.config.popularPosts.tmp.gaData[i].categories.split(" > ");
                        cat_spl = cat_spl[cat_spl.length -1];
                    }else{
                        cat_spl = _this.config.popularPosts.tmp.gaData[i].categories;
                    }
                    ...
                    if(postsCat_spl == "" || cat_spl == postsCat_spl ){gaData_opt.push(_this.config.popularPosts.tmp.gaData[i]);
                    }
                }
            }
        }
        ...
        for( i = 0; i < gaData_opt.length; i++ ){
            if( gaData_opt[i].path != postData.path )tmpList.push(gaData_opt[i]);
        }
        gaData_opt = tmpList;
