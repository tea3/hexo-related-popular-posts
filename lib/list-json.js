    ...
    if( Number(options.PPMixingRate) != 0 ){
        ...
        for( i = 0; i < _this.config.popularPosts.tmp.gaData.length; i++){
            if(_this.config.popularPosts.tmp.gaData[i].date != ""){
                var cat_spl = _this.config.popularPosts.tmp.gaData[i].categories.split(" > ")
                if(postsCat == "" || ( cat_spl && cat_spl[cat_spl.length -1] == postsCat) )gaData_opt.push(_this.config.popularPosts.tmp.gaData[i]);
            }
        }
